...
$owner = new WP_User($file->file_added_by);
...
<form enctype="multipart/form-data" name="<?php echo $action ?>" id="<?php echo $action ?>" method="post"
      action="<?php echo $form_url ?>" class="validate" style="max-width: 840px; margin: auto;">

    <?php
    if (!$in_widget) {
        if ($in_editor) {
            ?>
            <div style="float: right;"><a style="font-style:normal;"
                                          href="<?php echo add_query_arg('exform', ($exform ? '0' : '1')); ?>"><?php _e($exform ? 'Simple Form' : 'Extended Form', 'wp-filebase') ?></a>
            </div><h3 class="media-title"><?php echo $title ?></h3><?php
        } else {
            echo "<h2>" . $title . ' ';
            if (!$update) {
                ?><a style="font-style:normal; white-space: nowrap;"
                     href="<?php echo add_query_arg('exform', ($exform ? '0' : '1'), admin_url('admin.php?page=wpfilebase_files')) . '#' . $action; ?>"
                     class="add-new-h2"><?php _e($exform ? 'Simple Form' : 'Extended Form', 'wp-filebase') ?></a> <?php
            }
    ...
    <input type="hidden" name="file_id" id="file_id"
           value="<?php echo $update ? ($multi_edit ? $item_ids : $file->file_id) : ""; ?>"/>
    ...
    <div class="wpfb-upload-box">
        ...
        <div id="wpfilebase-upload-menu" class="admin-scheme-bgcolor-0">
            <a href="#" <?php echo($file->IsRemote() ? '' : 'class="admin-scheme-bgcolor-2 current"'); ?>
               onclick="return WPFB_switchFileUpload(0)"><?php $update ? _e('Upload Update', 'wp-filebase') : _e('Upload') ?></a>
            <a href="#" <?php echo($file->IsRemote() ? 'class="admin-scheme-bgcolor-2 current"' : ''); ?>
               onclick="return WPFB_switchFileUpload(1)"><?php _e('File URL') ?></a>
            <input type="hidden" name="file_is_remote" id="file_is_remote"
                   value="<?php echo($file->IsRemote() ? 1 : 0); ?>"/>
        </div>
        <div id="wpfilebase-upload-tabs">
            <div id="file-upload-wrap" <?php echo($file->IsRemote() ? 'class="hidden"' : ''); ?>>
                ...
                <div id="html-upload-ui">
                    ...
                    <?php printf(__('Maximum upload file size: %s.'/* def */), WPFB_Output::FormatFilesize(WPFB_Core::GetMaxUlSize())) ?>
                    <b>&nbsp;&nbsp;<a href="#" onclick="return alert(this.title) && false;"
                                      title="<?php printf(__('Ask your webhoster to increase this limit, it is set in %s.', 'wp-filebase'), 'php.ini'); ?>">?</a></b>
                ...
                if ($update || $exform) {
                    echo '<div>' . __('Rename') . ': ';
                    ?>
                    <input name="file_rename" id="file_rename" type="text"
                           value="<?php echo esc_attr($file->file_name); ?>" style="width:280px;"/><br/>
                    <?php
                    if ($update)
                        echo ' (' . $file->GetFormattedSize() . ', ' . wpfb_call('Download', 'GetFileType', $file->file_name) . ', MD5: <code>' . $file->file_hash . '</code>)';
                    echo '</div>';
                }
                ?>
            </div>
            <div id="file-remote-wrap" <?php echo($file->IsRemote() ? '' : 'class="hidden"'); ?>>
                <input name="file_remote_uri" id="file_remote_uri" type="text"
                       value="<?php echo esc_attr($file->file_remote_uri); ?>" style="width:100%"
                       placeholder="<?php _e('File URL', 'wp-filebase') ?>"/><br/>
                <fieldset>
                    <legend class="hidden"></legend>
                    <label><input type="radio" name="file_remote_redirect" value="0" <?php checked($file->IsLocal()); ?>
                                  onchange="jQuery('#wpfilebase-remote-scan-wrap').hide();"/><?php _e('Copy file into Filebase (sideload)', 'wp-filebase') ?>
                    </label>
                    <br/>
					<span style="text-overflow: ellipsis; white-space: nowrap"><label><input type="radio"
                                                                                             name="file_remote_redirect"
                                                                                             value="1" <?php checked($file->IsRemote()); ?>
                                                                                             onchange="jQuery('#wpfilebase-remote-scan-wrap').show();"/><?php _e('Redirect to URL', 'wp-filebase') ?></label>
						&nbsp;
						<span id="wpfilebase-remote-scan-wrap" class="<?php echo $file->IsRemote() ? '' : 'hidden'; ?>"><label><input
                                    type="checkbox" name="file_remote_scan" value="1"
                                    checked="checked"/><?php _e('Scan remote file (disable for large files)', 'wp-filebase') ?></label></span>
					</span>
                </fieldset>
            </div>
        </div>
    ...
    <table class="form-table">
            ...
            <tr>
                <?php if ($exform) { ?>
                    <th scope="row" valign="top"><label
                            for="file_upload_thumb"><?php _e('Thumbnail'/* def */) ?></label></th>
                    <td class="form-field" colspan="3"><input type="file" name="file_upload_thumb"
                        ...
                        <div style="<?php if (empty($file->file_thumbnail)) echo "display:none;"; ?>"
                            ...
                            <?php if ($update && !empty($file->file_thumbnail)) { ?> <label
                                for="file_delete_thumb"><?php _e('Delete') ?></label><input type="checkbox" value="1"
                                                                                            name="file_delete_thumb"
                                                                                            id="file_delete_thumb"
        ...
        <tr class="form-field">
            ...
            <td width="60%"><input name="file_display_name" id="file_display_name" type="text"
                                   value="<?php echo esc_attr($file->file_display_name); ?>"
                                   size="<?php echo ($in_editor || $in_widget) ? 20 : 40 ?>"/></td>
            <th scope="row" valign="top"><label for="file_version"><?php _e('Version') ?></label></th>
            <td width="40%"><input name="file_version" id="file_version" type="text"
                                   value="<?php echo esc_attr($file->file_version); ?>"
                                   size="<?php echo ($in_editor || $in_widget) ? 10 : 20 ?>"/></td>
        </tr>
        <tr class="form-field">
            <th scope="row" valign="top"><label for="file_author"><?php _e('Author') ?></label></th>
            <td><input name="file_author" id="file_author" type="text"
                       value="<?php echo esc_attr(!empty($file->file_author) ? $file->file_author : WPFB_Core::$settings->default_author); ?>"
                       size="<?php echo ($in_editor || $in_widget) ? 20 : 40 ?>"/></td>
            <?php if ($exform) { ?>
            ...
            <td><?php
                ...
                ?>
                <div class="wpfilebase-date-edit"><?php touch_time(1, 0); ?></div>
            </td>
        </tr>
        <tr class="form-field">
            <?php } ?>
            <th scope="row" valign="top"><label for="file_category"><?php _e('Category', 'wp-filebase') ?></label></th>
            <td><select name="file_category" id="file_category" class="postform wpfb-cat-select"
                        onchange="WPFB_formCategoryChanged();"><?php
                    echo WPFB_Output::CatSelTree(array('selected' => $file_category
                    , 'add_cats' => true
                    ))
                    ?></select></td>
            <?php if ($exform && !empty(WPFB_Core::$settings->licenses)) { ?>
                <th scope="row" valign="top"><label for="file_license"><?php _e('License', 'wp-filebase') ?></label>
                </th>
                <td><select name="file_license" id="file_license"
                            class="postform"><?php echo WPFB_Admin::MakeFormOptsList('licenses', $file ? $file->file_license : null, true) ?></select>
                </td>
            <?php } ?>
        </tr>
        ...
        <tr class="form-field">
            <?php if (!$in_editor) { ?>
                ...
                <td><input type="text" name="file_post_id" class="small-text" size="8"
                           style="width:60px; text-align:right;" id="file_post_id"
                           value="<?php echo esc_attr($file->file_post_id); ?>"/> <span id="file_post_title"
                                                                                        style="font-style:italic;"><?php if ($file->file_post_id > 0) echo get_the_title($file->file_post_id); ?></span>
                    <a href="javascript:;" class="button"
                       onclick="WPFB_PostBrowser('file_post_id', 'file_post_title');"><?php _e('Select') ?></a></td>
            <?php } else { ?>
                <td><input type="hidden" name="file_post_id" id="file_post_id"
                           value="<?php echo esc_attr($file->file_post_id); ?>"/></td>
            <?php } ?>
            <?php if ($exform) { ?>
            <th scope="row" valign="top"><label for="file_hits"><?php _e('Download Counter', 'wp-filebase') ?></label>
            </th>
            <td><input type="text" name="file_hits" class="small-text" id="file_hits"
                       value="<?php echo (int)$file->file_hits; ?>"/></td>
        </tr>
        <tr class="form-field">
            <?php if (WPFB_Core::$settings->platforms) { ?>
                <th scope="row" valign="top"><label
                        for="file_platforms[]"><?php _e('Platforms', 'wp-filebase') ?></label></th>
                <td><select name="file_platforms[]" size="40" multiple="multiple" id="file_platforms[]"
                            style="height: 80px;"><?php echo WPFB_Admin::MakeFormOptsList('platforms', $file ? $file->file_platform : null, true) ?></select>
                </td>
            <?php } else { ?>
                <th></th>
                <td></td><?php
            ...
            if (WPFB_Core::$settings->requirements) {
                ?>
                <th scope="row" valign="top"><label
                        for="file_requirements[]"><?php _e('Requirements', 'wp-filebase') ?></label></th>
                <td><select name="file_requirements[]" size="40" multiple="multiple" id="file_requirements[]"
        ...
        <tr>
            <?php if (WPFB_Core::$settings->languages) { ?>
                ...
                <td class="form-field"><select name="file_languages[]" size="40" multiple="multiple"
                                               id="file_languages[]"
            ...
            <th scope="row" valign="top"><label
                    for="file_direct_linking"><?php _e('Direct linking', 'wp-filebase') ?></label></th>
            <td>
                <fieldset>
                    <legend class="hidden"><?php _e('Direct linking') ?></legend>
                    <label title="<?php _e('Yes') ?>"><input type="radio" name="file_direct_linking"
                                                             value="1" <?php checked('1', $file->file_direct_linking); ?>/> <?php _e('Allow direct linking', 'wp-filebase') ?>
                    </label><br/>
                    <label title="<?php _e('No') ?>"><input type="radio" name="file_direct_linking"
                                                            value="0" <?php checked('0', $file->file_direct_linking); ?>/> <?php _e('Redirect to post', 'wp-filebase') ?>
                    </label>
        ...
        <tr <?php if (!$visual_editor) { ?>class="form-field"<?php } ?>>
            <th scope="row" valign="top"><label for="file_description"><?php _e('Description') ?></label>
                <?php if (!$in_widget && !$in_editor) { ?><br/><br/>
                    <a style="font-style:normal; font-size:9px; padding:3px; margin:0;"
                       href="<?php echo add_query_arg('visual_editor', ($visual_editor ? '0' : '1')) . '#' . $action; ?>"
                       class="add-new-h2"><?php _e($visual_editor ? 'Simple Editor' : 'Visual Editor', 'wp-filebase') ?></a>
                <?php } ?>
            </th>
            <td colspan="3">
                <?php
                if ($visual_editor) {
                    ...
                    ?>
                    <textarea name="file_description" id="file_description" rows="5" cols="50"
                              style="width: 97%;"><?php echo esc_html($file->file_description); ?></textarea>
                <?php } ?>
            </td>
        </tr>
        <tr class="form-field">
        ...
        <?php if ($exform) { ?>
            <tr>
                ...
                <td>
                    ...
                    <?php } else { ?>
                        <label><input type="radio" name="file_perm_explicit" value="0" <?php checked(true); ?>
                                      onchange="jQuery('#file_perm_wrap').hide()"/><?php _e('Inherit Permissions', 'wp-filebase') ?>
                            (<span id="file_inherited_permissions_label"></span>)</label>
                        <br/>
                        <label><input type="radio" name="file_perm_explicit" value="1"
                                      onchange="jQuery('#file_perm_wrap').show()"/><?php _e('Explicitly set permissions', 'wp-filebase') ?>
                        </label>
                    <?php } ?>
                    ...
                    ?>>
                        ...
                        <div
                            id="file_user_roles"><?php WPFB_Admin::RolesCheckList('file_user_roles', $user_roles) ?></div>
                    </div>
                </td>


                <th scope="row" valign="top"></th>
                <td><input type="checkbox" name="file_offline" id="file_offline"
                           value="1" <?php checked('1', $file->file_offline); ?>/> <label
                        for="file_offline"><?php _e('Offline', 'wp-filebase') ?></label></td>
        ...
        foreach ($custom_fields as $ct => $cn) {
            ...
            <tr class="form-field">
                ...
                <td colspan="3"><textarea name="<?php echo $hid; ?>" id="<?php echo $hid; ?>" rows="2" cols="50"
                                          style="width: 97%;"><?php echo (!$update && empty($file->$hid)) ? $custom_defaults[$ct] : esc_html($file->$hid); ?></textarea>
                </td>
            </tr> <?php
        }
        if (!empty($custom_fields)) {
            ?>
            <tr>
                <td colspan="4" style="text-align:right;margin:0;padding:0;"><a
                        href="<?php echo admin_url('admin.php?page=wpfilebase_sets#' . sanitize_title(__('Form Presets', 'wp-filebase'))); ?>"><?php _e('Manage Custom Fields', 'wp-filebase') ?></a>
                </td>
            </tr>
        <?php } ?>

        <?php do_action('wpfilebase_file_form_fields', $file); ?>
    </table>
    <p class="submit"><input type="submit" class="button-primary" id="file-submit" name="submit-btn"
                             value="<?php echo $update ? __('Update') : $title; ?>"
                             <?php if (false && !$in_editor) { ?>onclick="this.form.submit();
				return false;"<?php } ?>/></p>

    <?php
    //wpfb_loadclass('AdminGuiFiles');
    //WPFB_AdminGuiFiles::FilePageMetaBoxes($file);
    ?>
