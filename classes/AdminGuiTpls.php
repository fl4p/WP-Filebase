...
class WPFB_AdminGuiTpls
{
    ...
    static function initSamples()
    {
        ...
        self::$sample_file = new WPFB_File(array(
            ...
            'file_hits' => 3,
            'file_added_by' => get_current_user_id()
        ));
                ...
                <div class="wrap">
                    <h2><?php _e('Embed Templates', 'wp-filebase'); ?>
                        <?php if (empty(WPFB_Core::$settings->disable_css) && current_user_can('edit_themes')) { ?>
                            <a href="<?php echo admin_url('admin.php?page=wpfilebase_css'); ?>"
                               class="add-new-h2"><?php _e('Edit Stylesheet', 'wp-filebase'); ?></a>
                        <?php } ?>
                        <a href="<?php echo add_query_arg('iframe-preview', (int)empty($_GET['iframe-preview'])); ?>"
                           class="add-new-h2">iframe preview (uses actual theme styling)</a>
                    </h2>
                    ...
                    <form action="<?php echo remove_query_arg(array('action', 'type', 'tpl')) ?>" method="post"
                          onsubmit="return confirm('<?php _e('This will reset all File, Category and List Templates! Are your sure?', 'wp-filebase') ?>');">
                        <p>
                            <input type="submit" name="reset-tpls"
                                   value="<?php _e('Reset all Templates to default', 'wp-filebase') ?>" class="button"/>
                        </p></form>
        ...
        <table class="widefat post fixed" cellspacing="0">
            ...
            <?php foreach ($tpls as $tpl_tag => $tpl_src) {
                ...
                <tr id="tpl-<?php echo "$type-$tpl_tag" ?>" class="iedit" valign="top">
                    <th scope="row" class="check-column"><input type="checkbox" name="tpl[]"
                                                                value="<?php echo esc_attr($tpl_tag) ?>"/></th>
                    <td class="column-title">
                        <strong><a class="row-title" href="<?php echo $edit_link ?>"
                                   title="<?php printf(__('Edit &#8220;%s&#8221;'), $tpl_tag) ?>"><?php echo self::TplTitle($tpl_tag); ?></a></strong><br/>
                        <code>tpl=<?php echo $tpl_tag; ?></code>
                        <div class="row-actions"><span class='edit'><a href="<?php echo $edit_link ?>"
                                                                       title="<?php _e('Edit this item') ?>"><?php _e('Edit') ?></a></span>
                            <?php if (!in_array($tpl_tag, self::$protected_tags)) { ?><span class='trash'>| <a
                                    class='submitdelete' title='<?php _e('Delete this item permanently') ?>'
                                    href='<?php echo wp_nonce_url(add_query_arg(array('action' => 'del', 'type' => $type, 'tpl' => $tpl_tag)), 'del-' . $type, 'wpfb-tpl-nonce') . '#' . $type; ?>'><?php _e('Delete') ?></a>
                                </span><?php } ?>
                        </div>
                    </td>
                    <td>
                        <div class="entry-content wpfilebase-tpl-preview">
                            <div id="tpl-preview_<?php echo $tpl_tag ?>">
                                <?php if (!empty($_GET['iframe-preview'])) { ?>
                                    <iframe
                                        src="<?php echo admin_url("admin.php?wpfilebase-screen=tpl-preview&type=$type&tag=$tpl_tag"); ?>"
    ...
    static function TplForm($type, $tpl_tag = null)
    {
        ...
        <form action="<?php echo remove_query_arg(array('action', 'type', 'tpl')) . '#' . $type ?>" method="post">
            ...
            <?php if ($new) { ?>
                <p>
                    <label
                        for="tpltag"><?php _e('Template Tag (a single word to describe the template):', 'wp-filebase') ?></label>
                    <input type="text" name="tpltag"
                           value="<?php if (!empty($_POST['tpltag'])) echo esc_attr($_POST['tpltag']); ?>" tabindex="1"
                           maxlength="20"/>
                </p>
                ...
                <table class="form-table">
                    <tr class="form-field">
                        <th scope="row" valign="top"><label
                                for="tpl-list-header"><?php _e('Header', 'wp-filebase') ?></label></th>
                        <td width="100%">
                            <textarea id="tpl-list-header" name="tpl-list-header" cols="70"
                                      rows="<?php echo(max(2, count(explode("\n", $tpl->header))) + 3); ?>" wrap="off"
                                      class="codepress html wpfilebase-tpledit"
                                      onkeyup="WPFB_PreviewTpl(this, '<?php echo $type ?>')"
                                      onchange="WPFB_PreviewTpl(this, '<?php echo $type ?>')"><?php echo htmlspecialchars($tpl->header) ?></textarea><br/>
                        </td>
                    </tr>
                    <tr class="form-field">
                        <th scope="row" valign="top"><label
                                for="tpl-list-cat-tpl"><?php _e('Category Template', 'wp-filebase') ?></label></th>
                        <td width="">
                            <select id="tpl-list-cat-tpl" name="tpl-list-cat-tpl"
                                    onchange="WPFB_PreviewTpl(this, '<?php echo $type ?>')"><?php echo WPFB_Admin::TplDropDown('cat', $tpl->cat_tpl_tag); ?></select>
                        </td>
                    </tr>
                    <tr class="form-field">
                        <th scope="row" valign="top"><label
                                for="tpl-list-file-tpl"><?php _e('File Template', 'wp-filebase') ?></label></th>
                        <td>
                            <select id="tpl-list-file-tpl" name="tpl-list-file-tpl"
                                    onchange="WPFB_PreviewTpl(this, '<?php echo $type ?>')"><?php echo WPFB_Admin::TplDropDown('file', $tpl->file_tpl_tag); ?></select>
                        </td>
                    </tr>
                    <tr class="form-field">
                        <th scope="row" valign="top"><label
                                for="tpl-list-footer"><?php _e('Footer', 'wp-filebase') ?></label></th>
                        <td>
                            <textarea id="tpl-list-footer" name="tpl-list-footer" cols="70"
                                      rows="<?php echo(max(2, count(explode("\n", $tpl->footer))) + 3); ?>" wrap="off"
                                      class="codepress html wpfilebase-tpledit"
                                      onkeyup="WPFB_PreviewTpl(this, '<?php echo $type ?>')"
                                      onchange="WPFB_PreviewTpl(this, '<?php echo $type ?>')"><?php echo htmlspecialchars($tpl->footer) ?></textarea><br/>
                        </td>
                    </tr>
            ...
            <?php } else { ?>
                <p>
                    ...
                    <textarea id="<?php echo $code_id ?>" cols="70"
                              rows="<?php echo(max(2, count(explode("\n", $tpl_code))) + 3); ?>" wrap="off"
                              name="tplcode" class="codepress html wpfilebase-tpledit"
                              onkeyup="WPFB_PreviewTpl(this, '<?php echo $type ?>')"
                              onchange="WPFB_PreviewTpl(this, '<?php echo $type ?>')"><?php echo htmlspecialchars($tpl_code) ?></textarea><br/>
                    <?php wpfb_loadclass('Models');
                    echo WPFB_Models::TplFieldsSelect($code_id, false, $cat) ?>
                </p>
            <?php } ?>

            <p class="submit"><input type="submit" name="submit" class="button-primary"
                                     value="<?php echo esc_attr__($new ? 'Add Template' : 'Submit Template Changes', 'wp-filebase') ?>"/>
            </p>
        </form>
