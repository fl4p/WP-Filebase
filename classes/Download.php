...
class WPFB_Download
{
    static function RefererCheck()
    {
        ...
        foreach ($allowed_referers as $ar) {
            ...
            if (@strpos($referer, $ar) !== false
                || @strpos($referer, $ar_host) !== false
            ) {
                return true;
            }
        }
    ...
    static function GetFileType($name)
    {
        ...
        switch (strtolower($name)) {
            ...
            case 'docx':
                return 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'; //wtf? ;)
            case 'doc':
            ...
            case 'notebook':
                return 'application/notebook';

            case 'gadget':
                return 'application/x-windows-gadget';
...
// returns true if range download should be supported for the specified file/file type
    ...
    public static function ParseRangeHeader($file_size)
    {
        ...
        $http_range = isset($_SERVER['HTTP_RANGE']) ? $_SERVER['HTTP_RANGE']
            : '';
        if ( ! empty($http_range) && strpos($http_range, 'bytes=') !== false
            && strpos($http_range, ',') === false
        ) // multi-range not supported (yet)!
        {
            $range = array_map('trim',
                explode('-', trim(substr($http_range, 6))));
    ...
    static function SendFile($file_path, $args = array())
    {
        ...
        header("Content-Type: " . $file_type . ((strpos($file_type, 'text/')
                !== false) ? '; charset=' : ''));    // charset fix
        ...
        if ( ! $no_cache) {
            ...
            $if_mod_since  = ! empty($_SERVER['HTTP_IF_MODIFIED_SINCE'])
                ? $_SERVER['HTTP_IF_MODIFIED_SINCE'] : false;
            $if_none_match = ! empty($_SERVER['HTTP_IF_NONE_MATCH']) ? ($etag
        ...
        $request_file_name
            = basename(urldecode(parse_url($_SERVER['REQUEST_URI'],
            PHP_URL_PATH)));
    ...
    static function SideloadFile(
        $url,
        $dest_path,
        $progress_bar_or_callback = null
    ) {
        if (WP_DEBUG_LOG) {
            error_log("WPFB: SideloadFile $url to $dest_path");
        }
        ...
        $total_size = $is_local ? filesize($url)
            : self::getHttpStreamContentLength($rh);
        ...
        if ($fh === false) {
            ...
            return array(
                'error' => sprintf('Could not create file %s!', $dest_path)
            );
        }
        ...
        while ( ! feof($rh)) {
            if (($s = fwrite($fh, fread($rh, 65536))) === false) {
                ...
                return array(
                    'error' => sprintf('Writing to file %s failed!', $dest_path)
                );
            }
            $size += $s;
    ...
    private static function getFirstIp($comma_list) {
        $p = strpos($comma_list, ',');
        if($p === false)
            return $comma_list;
        else
            return substr($comma_list, 0, $p);
    }

    public static function GetClientIp() {
        $headers = function_exists( 'apache_request_headers' ) ? apache_request_headers() : $_SERVER;
        if ( array_key_exists( 'X-Forwarded-For', $headers ) && filter_var( self::getFirstIp($headers['X-Forwarded-For']), FILTER_VALIDATE_IP ) ) {
            $the_ip = $headers['X-Forwarded-For'];
        } elseif ( array_key_exists( 'HTTP_X_FORWARDED_FOR', $headers ) && filter_var( self::getFirstIp($headers['HTTP_X_FORWARDED_FOR']), FILTER_VALIDATE_IP ) ) {
